...
- ✨ **版本号自动递增**
  ...
  - 新增「自动递增版本号」配置选项，位于目标目录区域（默认开启）
  - 启用后，每次修改 theme\*.json 文件时会自动将文件中的 `version` 字段值加 1
  - 支持所有操作类型：批量新增、导入、批量删除、单个删除
  - 可通过界面复选框随时开启/关闭此功能

- ✨ **操作日志增强**
  - 操作日志中新增版本变化记录
  - 显示格式：`版本变化: X 个文件`，然后列出每个文件的 `旧版本 → 新版本`
  - 只有在启用自动递增版本号时才会记录版本变化信息
  - 帮助用户追踪每次操作对文件版本的影响

### 改进

- 🔧 优化了版本号提取和递增逻辑，确保准确识别和更新 JSON 中的 version 字段
- 🔧 改进了操作日志的详细程度，增加了版本变化追踪
- 🔧 优化了用户界面，版本号配置选项位置合理，操作便捷

### 技术改进

- 添加了 `extract_version()` 函数用于提取 JSON 文件中的版本号
- 添加了 `VersionChange` 结构体记录版本变化信息
- 修改了 `write_operation_log()` 函数支持版本变化记录
- 在所有操作函数中添加了版本变化跟踪逻辑
- 前端添加了 `autoIncrementVersion` 状态管理
...
- ✨ **操作日志**
  ...
  - 新增操作记录新增的本地栏目 ID 和国网栏目 ID
  - 删除操作记录删除的本地栏目 ID 和国网栏目 ID
